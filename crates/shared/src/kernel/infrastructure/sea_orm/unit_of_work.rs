...
use crate::kernel::{
    ...
    infrastructure::sea_orm::log_database_error,
};

/// Builds a service's transaction context bound to an open transaction.
///
/// The one piece of a SeaORM unit of work that differs per service. Each
/// service implements this once for its context and plugs it into
/// [`SeaOrmUnitOfWork`].
...
impl<F: SeaOrmTransactionContextFactory> UnitOfWork for SeaOrmUnitOfWork<F> {
        ...
        W: for<'r, 'tx> FnOnce(&'r Self::TransactionContext<'tx>) -> Work<'r, T, E> + Send,
    {
        ...
        match outcome {
            ...
            Err(error) => {
                // Dropping the transaction would also roll back. Explicit so a
                // rollback failure surfaces in the logs.
...
mod tests {
    ...
    /// Work error with a domain variant of its own, the shape a use case takes
    /// when its work can fail beyond its repositories.
    #[derive(Debug, PartialEq, Eq, thiserror::Error)]
    enum NoteError {
        #[error("work failed")]
        WorkFailed,
        #[error("concurrent work")]
        Conflict,
        #[error("unexpected failure")]
        Internal,
    }

    impl From<RepositoryError> for NoteError {
    ...
    /// An unrecognized unique violation is unexpected, not automatically a
    /// retryable conflict. Earlier writes must still roll back.
    #[tokio::test]
    async fn rolls_back_unrecognized_duplicate_insert_as_internal() {
        let unit_of_work = unit_of_work().await;
        ...
        assert_eq!(outcome, Err(RepositoryError::Internal));
        assert!(stored_ids(&unit_of_work).await.is_empty());
    }
}
