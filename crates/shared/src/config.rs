...
impl AppConfig {
    ...
    fn apply_env<T: DeserializeOwned>(
        ...
        // A null default erases the Option's inner type. Let T accept the
        // literal string first, preserving numeric and boolean-looking text for
        // Option<String>, before trying scalar inference.
        if template_leaf.is_null() {
            insert_at_path(tree, path, Value::String(raw.to_owned()));
            if serde_json::from_value::<T>(tree.clone()).is_ok() {
                return Ok(());
            }
        }
...
/// is the field's default value. For a null template (an `Option` field), the
/// caller tries a string against the target type first. This fallback guesses
/// bool, then number, then string.
