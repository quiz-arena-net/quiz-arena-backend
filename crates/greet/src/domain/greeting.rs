...
///
/// Guaranteed to contain 1 to 32 Unicode alphanumeric characters, with no
/// spaces. Length bounds match the proto field's validation rules. Accepted
/// names are preserved exactly, without trimming or Unicode normalization.
...
impl SenderName {
    ...
    pub(crate) fn new(name: impl Into<String>) -> Result<Self, SenderNameError> {
        ...
        if !name.chars().all(char::is_alphanumeric) {
            return Err(SenderNameError::InvalidCharacter);
        }
        Ok(Self(name))
    }
...
impl Greeting {
    ...
    ///
    /// For repository implementations only. Trusts the stored count, which this
    /// aggregate produced in the first place.
...
/// Expected reasons a greeting history cannot be added.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub(crate) enum AddGreetingRejection {
    #[error("greeting history already exists for this sender")]
    AlreadyExists,
}

/// Port for persisting greeting histories.
///
/// Failures cross the boundary as [`RepositoryError`], with typed rejections
/// for adding a greeting. The application translates these operation outcomes
/// into use-case errors and decides whether to retry.
#[async_trait]
...
mod tests {
    ...
    fn rejects_invalid_characters() {
        for name in [
            ...
            assert_eq!(
                SenderName::new(name),
                Err(SenderNameError::InvalidCharacter),
                "{name:?}"
            );
        }
    }
    ...
    #[test]
    fn preserves_accepted_names_exactly() {
        for name in [
            "alice",
            "Alice",
            "alice123",
            ...
            assert_eq!(SenderName::new(name).unwrap().as_str(), name);
        }
    }
