...
pub(crate) enum GreetOutput {
    Recorded { sender: String, returning: bool },
    Rejected(GreetRejection),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub(crate) enum GreetRejection {
...
impl From<SenderNameError> for GreetRejection {
    ...
    for<'tx> U::TransactionContext<'tx>: GreetTransactionContext,
{
    ...
    async fn execute(&self, input: GreetInput) -> Result<GreetOutput, GreetError> {
        let sender = match SenderName::new(input.sender) {
            Ok(sender) => sender,
            Err(error) => {
                let rejection = GreetRejection::from(error);
                info!(%rejection, "rejected greeting");
                return Ok(GreetOutput::Rejected(rejection));
            }
        };
        ...
        let outcome = loop {
            ...
            match outcome {
                Err(error @ GreetError::Conflict) if attempt < MAX_ATTEMPTS => {
...
mod tests {
    ...
    async fn rejects_invalid_names_before_accessing_repositories() {
        for (sender, expected) in [
            (String::new(), GreetRejection::EmptySenderName),
            (
                "alice bob".to_owned(),
                GreetRejection::SenderNameContainsInvalidCharacter,
            ),
            (
                "あ".repeat(33),
                GreetRejection::SenderNameTooLong { length: 33 },
            ),
        ] {
            let (interactor, attempts) = interactor(0);
            let output = interactor.execute(GreetInput { sender }).await.unwrap();
            assert_eq!(output, GreetOutput::Rejected(expected));
